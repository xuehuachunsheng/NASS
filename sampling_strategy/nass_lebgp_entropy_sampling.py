...
class NASSLEBGPEntropyStrategy(BatchSampleSelectionStratgy):
    ...
    def select(self) -> np.ndarray:
        CONF = self.CONF
        CONF.logger.info(f"NASS LEBGPEntropy Selecting samples. ")
        ...
        for i in range(n_samples):
            ...
            tildePN[i] = tildePNx
            lebgp_entropies[i] = -np.sum((tildePNx + 1e-6) * np.log(tildePNx + 1e-6))
        ...
        for i in range(len(pred_C_idx)):
            # 按照MSC值大小进行分组并排序
            pred_C_idx[i] = [(id, lebgp_entropies[id]) for id in pred_C_idx[i]]
