...
class NASSEntropyStrategy(BatchSampleSelectionStratgy):
    ...
    def select(self) -> np.ndarray:
        CONF = self.CONF
        CONF.logger.info(f"NASS Entropy Selecting samples. ")
        ...
        CONF.logger.info("Step 2.2. Compute Entropy")
        ...
        for i in range(n_samples):
            ...
            tildePN[i] = tildePNx
            entropies[i] = -np.sum((fx + 1e-6) * np.log(fx + 1e-6))
        ...
        for i in range(len(pred_C_idx)):
            # 按照MSC值大小进行分组并排序
            pred_C_idx[i] = [(id, entropies[id]) for id in pred_C_idx[i]]
