...
class LEBGPLeastConfidenceStrategy(BatchSampleSelectionStratgy):
    ...
    def select(self) -> np.ndarray:
        CONF = self.CONF
        CONF.logger.info("LeastConfidence using Local Empirical Bayes with Global Prior Correction.")
        ...
        lebgp_confidences = {}
        for k in tildePN:
            lebgp_confidences[k] = np.max(tildePN[k])

        # 按照margin值从小到大排序
        CONF.logger.info("Step 3. Sort samples by LEBGP Least Confidence values and return respective idx with top-k values.")
        lebgp_confidences = list(lebgp_confidences.items())
        lebgp_confidences = sorted(lebgp_confidences, key=lambda x: x[1])
        top_k_idx = [x[0] for x in lebgp_confidences[:self.budget]]
