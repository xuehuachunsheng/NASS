...
class NASSMarginStrategy(BatchSampleSelectionStratgy):
    ...
    def select(self) -> np.ndarray:
        CONF = self.CONF
        CONF.logger.info(f"NASS Margin Selecting samples. ")
        ...
        CONF.logger.info("Step 2.2. Compute Margin")
        ...
        for i in range(n_samples):
            ...
            tildePN[i] = tildePNx
            sorted_probs = np.sort(fx)
            margins[i] = sorted_probs[-1] - sorted_probs[-2]
        ...
        for i in range(len(pred_C_idx)):
            # 按照MSC值大小进行分组并排序
            pred_C_idx[i] = [(id, margins[id]) for id in pred_C_idx[i]]
