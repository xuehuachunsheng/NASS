...
from utils import stratify,compute_bc,true_sample_idx_each_class,compute_bc_knownpopulation
# Renormalization group multiscale complexity
class GASSVarianceReductionStrategy(BatchSampleSelectionStratgy):
    ...
    def select(self) -> np.ndarray:
        ...
        for i in range(len(pred_C_idx)):
            # 按照var值大小进行分组并排序
            pred_C_idx[i] = [(id, unlabeled_vars[id]) for id in pred_C_idx[i]]
