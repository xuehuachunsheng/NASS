...
class LEBGPEntropyStrategy(BatchSampleSelectionStratgy):
    ...
    def select(self) -> np.ndarray:
        CONF = self.CONF
        CONF.logger.info("Entropy using Local Empirical Bayes with Global Prior Correction.")
        ...
        CONF.logger.info("Step 2.2. Compute LEBGP Entropy")
        ...
        lebgp_entropies = {}
        for k in tildePN:
            lebgp_entropies[k] = -np.sum((tildePN[k] + 1e-6) * np.log(tildePN[k] + 1e-6))

        # 按照Entropy值从大到小排序
        CONF.logger.info("Step 3. Sort samples by LEBGP entropy values and return respective idx with top-k values.")
        lebgp_entropies = list(lebgp_entropies.items())
        lebgp_entropies = sorted(lebgp_entropies, key=lambda x: -x[1])
        top_k_idx = [x[0] for x in lebgp_entropies[:self.budget]]
